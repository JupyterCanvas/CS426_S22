#!/usr/bin/env python3

# restart containers, vncservers, websockify instances after a reboot
# (can't get all services to start correctly at boot, manual workaround)
# ./reboot.py cs123/cs123-conts.txt

import sys # for sys.exit, sys.argv
import logging 
import argparse # for adding CLI tags/help
import textwrap # for formatting argparse help
import subprocess
from subprocess import PIPE

# setup CLI args and help
parser = argparse.ArgumentParser(
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description=textwrap.dedent('''\
        Resart user container services from list of instances:
          (run create_containers.py to generate instance_list)
          Starts: 
            container / container networking test services
            container vncservers
            server websockify instances to containers
        '''),
        epilog='\
        ')

parser.add_argument('instance_list', type=argparse.FileType('r'), 
                    help='List of container instances in a text file \
                    run create_containers.py to generate cs123/cs123-conts.txt')
args = parser.parse_args()


# setup logging:
LOG_FILE= ''
FORMAT = ''
logging.basicConfig(level=logging.INFO,
                     format = FORMAT)

#CONSOLE LOGGING
consoleHandler = logging.StreamHandler()
consoleHandler.setLevel(logging.INFO)

#INFO LOGGER
logger = logging.getLogger('dev')
logger.setLevel(logging.INFO)
logger.addHandler(consoleHandler)
# to prevent doubling of error messages in output:
logger.propagate = False
        
# start container instances with systemd
def start_instances(usernames):
    
    for user in usernames:
        # testcont@.service starts cont@.service and checks networking
        depinst = f"cont@{user}.service"
        instance = f"testcont@{user}.service"
        
        # start testcont for instance (starts cont for instance)
        p = subprocess.run(["systemctl", "start", instance], stdout=PIPE)
        if p.returncode == 0:
            logger.info("Container " + depinst + " started for " + user)
        else:
            logger.info("Error starting container for " + user)

def start_vnc(usernames):
    
    for user in usernames:
        instance = f"vnc@{user}.service"
        # start vncserver for instance
        p = subprocess.run(["systemctl", "start", instance], stdout=PIPE)
        if p.returncode == 0:
            logger.info("VNC server started with " + instance)
        else:
            logger.info("Error starting VNC server with " + instance)
   
    # additions to sysctl.conf and 
    # patch to TurboVNC vncserver executable fixed following issue 
    # - no it didn't, putting check back in:
    # I think there is a race condition happening
    # 2-3 of the 5 vncserver services run less than 50 tasks
    # others run more than 70 tasks to fully setup vnc desktop
    # stopping and starting services with less than 50 tasks reported 
    # fixes the issue:
    for user in usernames: 
        p = subprocess.run(["./fixvnc.sh", user], stdout=PIPE)
        if p.returncode == 0:
            logger.info("VNC server checked for " + user + ":")
            logger.info(p.stdout.decode('utf-8'))
        else:
            logger.info("Error checking VNC server for  " + user + ":")
            logger.info(p.stdout.decode('utf-8'))

def start_ws(usernames): 
    for user in usernames: 
        instance = f"ws@{user}.service"
        p = subprocess.run(["systemctl", "start", instance], stdout=PIPE)
        if p.returncode == 0: 
            logger.info("Websockify started for " + user)
        else: 
            logger.info("Error starting websockify for " + user)


def main():
    with open(args.instance_list.name) as fin:
        instances = [inst.strip() for inst in fin]
    
    usernames = []
    for inst in instances: 
        user = inst.split(':', 1)[0]
        usernames.append(user)

    start_instances(usernames)

    start_vnc(usernames)
    
    start_ws(usernames)

if __name__ == "__main__":
    main()

